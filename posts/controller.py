...
def update_post(post_id: int,post: PostCreate, service: PostService = Depends(get_service)):
    return service.update_full(post_id, post)

@router.patch("/", response_model=PostResponse)
def update_post(post_id: int,post: PostUpdate, service: PostService = Depends(get_service)):
